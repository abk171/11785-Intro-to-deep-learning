    ...
    def forward(self, x, h_0=None):
        ...
        for i in range(seq_len):
            ...
            for j, rnn in enumerate(self.rnn):
                h_prev_t = self.hiddens[-1][j,:,:]
                h_t = rnn.forward(x_i, h_prev_t)
                hidden[j,:,:] = h_t
                x_i = h_t
                
            self.hiddens.append(hidden.copy())
        
        last_out = self.hiddens[-1][-1]
        logits = self.output_layer.forward(last_out)
    ...
    def backward(self, delta):
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for t in reversed(range(seq_len)):
            for i, rnn in reversed(list(enumerate(self.rnn))):
                h_prev_l = self.hiddens[t+1][i-1] if i > 0 else self.x[:,t,:]
                h_prev_t = self.hiddens[t][i]
                dx, dh_prev_t = rnn.backward(dh[i], self.hiddens[t+1][i], h_prev_l, h_prev_t)
                dh[i] = dh_prev_t
                if i > 0:
                    dh[i-1] += dx

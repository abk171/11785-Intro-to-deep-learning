...
class CTC(object):
    ...
    def get_forward_probs(self, logits, extended_symbols, skip_connect):
        ...
        for i in range(1,T):
            ...
            for j in range(1,S):
                alpha[i,j] = alpha[i-1,j-1] + alpha[i-1,j] + (alpha[i-1, j-2] if skip_connect[j] else 0)
    ...
    def get_backward_probs(self, logits, extended_symbols, skip_connect):
        ...
        for i in reversed(range(T-1)):
            beta[i,-1] = beta[i+1,-1] * logits[i+1,extended_symbols[-1]]
            for j in reversed(range(S-1)):
                beta[i,j] = beta[i+1,j] * logits[i+1,extended_symbols[j]] + \
                            beta[i+1,j+1] * logits[i+1,extended_symbols[j+1]] + \
                            (beta[i+1,j+2] * logits[i+1,extended_symbols[j+2]] \
                            if (j < S-3 and skip_connect[j+2]) else 0)
...
class CTCLoss(object):
    ...
    def forward(self, logits, target, input_lengths, target_lengths):
        ...
        for batch_itr in range(B):
            ...
            # -------------------------------------------->
            # TODO
            t = self.target[batch_itr][:target_lengths[batch_itr]]
            l = self.logits[:input_lengths[batch_itr],batch_itr,:]

            es, sc = self.ctc.extend_target_with_blank(t)
            self.extended_symbols.append(es)
            alpha = self.ctc.get_forward_probs(l,es,sc)
            beta = self.ctc.get_backward_probs(l,es,sc)
            ...
            for i in range(T):
                for j in range(S):
                    total_loss[batch_itr] -= gamma[i,j]*np.log(l[i,es[j]])
            # <---------------------------------------------
    ...
    def backward(self):
        ...
        for batch_itr in range(B):
            ...
            T,S = self.gammas[0].shape

            for i in range(T):
                for j in range(S):
                    dY[i,j,self.extended_symbols[batch_itr][j]] -= self.gammas[batch_itr][i,j] / l[i,self.extended_symbols[batch_itr][j]]

...
class Attention:
        ...
        def forward(self, X):
            ...
            attn_mask    = torch.triu(torch.ones(T,T), diagonal=1)# TODO

            # Calculate/normalize Attention Scores
            self.D_k = self.K.shape[-1] ** 0.5
            self.A_sig   =  self.softmax.forward(self.A_w.masked_fill(attn_mask.bool(), -torch.inf) / self.D_k)# TODO
        ...
        def backward(self, dLdXnew):
            ...
            dLdA_w         = (1 / self.D_k) * self.softmax.backward(dLdA_sig) # TODO
            ...
            self.dLdWq     = torch.sum(torch.bmm(self.X.transpose(1,2),self.dLdQ), dim=0)# TODO
            self.dLdWv     = torch.sum(torch.bmm(self.X.transpose(1,2),self.dLdK), dim=0)# TODO
            self.dLdWk     = torch.sum(torch.bmm(self.X.transpose(1,2),self.dLdQ), dim=0)# TODO
